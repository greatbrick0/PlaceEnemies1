...
public class BoulderAbility : Ability
{
    ...
    private void MakeProjectile(Vector3 targetPosition)
    {
        ...
        boulderRef.transform.GetComponent<BoulderScript>().moveDirection = targetPosition - user.transform.position;
        boulderRef.transform.GetComponent<BoulderScript>().team = user.GetComponent<CombatBody>().team;
        boulderRef.transform.GetComponent<BoulderScript>().FaceForward();
    }
}
